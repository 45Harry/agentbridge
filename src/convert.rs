...
mod tests {
    ...
    fn test_output_ends_with_newline_so_appends_do_not_corrupt() {
        ...
        for path in [
            ClaudeCodeConverter::new().convert(&session, tmp.path()).unwrap(),
            CodexCliConverter::new().convert(&session, tmp.path()).unwrap(),
        ] {
    ...
    fn test_codex_rollout_path_is_deterministic() {
        ...
        let p1 = c.convert(&session, tmp.path()).unwrap();
        let p2 = c.convert(&session, tmp.path()).unwrap();
    ...
    fn test_codex_resume_cmd_extracts_bare_uuid() {
        ...
        let path = converter.convert(&session, tmp.path()).unwrap();
    ...
    fn test_converter_with_empty_messages() {
        ...
        let result = cc.convert(&session, tmp.path());
        assert!(result.is_ok());
